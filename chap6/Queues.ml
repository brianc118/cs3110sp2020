...
module TwoListQueue : Queue = struct
  ...
  let rep_ok q =
    if List.length q.front = 0 && List.length q.back > 0 then
      raise (Failure "rep_not_ok")
    else q

  let empty = rep_ok { front = []; back = [] }

  let is_empty q =
    match rep_ok q with { front = []; back = [] } -> true | _ -> false

  let singleton x = rep_ok { front = [ x ]; back = [ ] }
  ...
  let enqueue x q = norm { (rep_ok q) with back = x :: q.back } |> rep_ok

  let peek q =
    match rep_ok q with { front = [] } -> None | { front = x :: _ } -> Some x

  let dequeue q =
    match rep_ok q with
    | { front = []; _ } -> None
    | { front = _ :: xs; back } -> Some (norm { front = xs; back } |> rep_ok)

  let to_list q =
    let { front; back } = rep_ok q in
    front @ List.rev back
end
